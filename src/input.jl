...
function parameter_homotopy(
    ...
    parameters = (
        isa(F, MPPolyInputs) ?
        throw(ArgumentError("You need to pass `parameters=...` as a keyword argument.")) :
        nothing
    ),
    generic_parameters = nothing,
    start_parameters = generic_parameters,
    ...
    if F isa System && length(F.parameters) != length(p₀)
        throw(ArgumentError("Number of parameters doesn't match!"))
    end
...
#         ))
#     end
#     if !(length(p₁) == length(p₀) == length(F.parameters))
#         throw(ArgumentError("Number of parameters doesn't match!"))
#     end
#
...
#
#     (
#      input = ParameterSystemInput(F, nothing, p₁, p₀, γ₁, γ₀),
#      startsolutions = startsolutions,
#     )
# end
